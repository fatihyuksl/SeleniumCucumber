...
public class GuruStepDefinition {
    ...
    public void sutunundakiTumDegerleriYazdirir(String basliklar) {
        int index = 0;
        for (int i = 0; i < guru.basliklar.size(); i++) {
            if (guru.basliklar.get(i).getText().equals(basliklar)) { //Feature'dan gelen string değeri i degerine eşitledik
